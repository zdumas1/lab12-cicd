...
from e2e_tests.common.methods import genz_anonymize
...
def test_given_anonymize_called_with_genz_then_expected_valid_response_returned(
    presidio_anonymizer_service_url,
):
    payload = {
    ...
    response = genz_anonymize(presidio_anonymizer_service_url, payload)

    # Because output is random, we only assert success
